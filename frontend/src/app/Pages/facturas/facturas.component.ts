...
export class FacturasComponent implements OnInit {
  ...
  ObtenerFacturas() {
    fetch(`${api_url.factura}`).then(res => {
  ...
  editarFactura(venta: any) {
    fetch(`${api_url.factura}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', body: JSON.stringify(venta) } }).then(res => {
      if (!res.ok) throw new Error('Error en la peticion');
      return res.json();
    }).then(data => {
      alert(data);
  ...
  eliminarFactura(venta: any) {
    fetch(`${api_url.factura}/${venta.id}`, { method: 'DELETE' }).then(res => {
      if (!res.ok) throw new Error('Error en la peticion');
      return res.json();
    }).then(data => {
      alert(data);
